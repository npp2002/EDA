...
#some caveats to this rudimentary relationship-number of negative ratings could show a skewed relationship if one game is more popular than another.
...
```
```{r}
steam_games_2 = steam_games_final %>%
  group_by(release_date) %>%
  summarize(mean_age=mean(age)) %>%
...
```
```{r}
steam_games_3 = steam_games_final %>%
  group_by(release_date) %>%
  summarize(release_date, age) %>%
  arrange(desc(release_date))
steam_games_3
```
```{r}
ggplot(data=steam_games_final, aes(x=release_date, y=age))+
  geom_point()+
  labs(x="Release Date", y="Required Age")
...
ggplot(steam_games_5, aes(x=platform, y=owners)) +
  geom_point() +
