...
co7 = steam_games_final %>%
  select(positive_ratings,average_playtime) %>%
  filter(positive_ratings!= 0,average_playtime != 0)

#'0' %in% co7$positive_ratings
#check why i am still getting 0 value

ggplot(co7,aes(x=positive_ratings,y=average_playtime)) +
  geom_point() +  
   xlab("Number of Positive Ratings") + ylab("Average Playtime") + xlim(0,500000)

#do a linear correlation, or a further question exploring a third variable within this scatter plot
